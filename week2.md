...
- 수행 결과 :
<img width="559" height="487" alt="image" src="https://github.com/user-attachments/assets/c3b901ed-4d9a-4817-bbc2-9c38d1508421" />

- 비록 퀘스트를 한 번 밖에 수행하지 못했지만, 정말 도움이 되는 꿀팁을 많이 알려줘서 좋았습니다.
- 평소에 멀티 커서 단축키를 굉장히 유용하게 사용하는데, 지금까지 안써본 멀티 커서 단축키도 새로 배우고,
  Todo Tree extension 같은 것도 존재를 몰랐는데 알게 되어서 좋았습니다.
- AI를 활용하지 않았다면 불편함을 느끼는 부분에 대해 직접 검색하거나,
  제가 비효율적으로 하는 걸 보고 다른 사람이 '이렇게 하면 편해~'라고 알려주지 않는 이상 스스로 알게 되기는 어려운데,
  비교적 적은 노력을 통해 생산성을 높일 수 있었던 것 같습니다.
- 이 경험을 봤을 때, *나한테 있으면 좋지만 정확하게 어떤 도구가 도움이 될지 모를 때* AI를 활용하면 좋은 것 같습니다.
...
- 수행 결과 : 
![quest-result](https://raw.githubusercontent.com/nuyheat/nuyheat/main/result.png)
- 느낀점:
    - 저와의 대화를 기반으로 했는지 매우 현 상황에 잘 맞는 3행시를 지어 줬습니다.
    - 릴프 진행할 때마다 항상 느끼는 거지만 과제보다 덜 우선하게 되는 경향이 강했는데, 이번에도 그렇게 되어 아쉬움이 많이 남습니다..!
...
#### K005 : 4️⃣ 저메추 추천 프롬프트 작성하고(오늘의 기분 바탕) 내용 공유하기
- 선택한 이유:
...
#### J018: 1️⃣ 매일 ai에게 밸런스 게임 주제 물어보고 슬랙에 올리기
##### 미션 선택 이유
- 가벼운 주제를 뽑아서 공유하면 미션을 수행하는 중간에 잠시 머리를 식힐 수 있지 않을까 싶어 선택했습니다.
- 비슷한 느낌의 3번 미션도 있지만, 더 다양한 내용을 공유하는 게 휴식에 도움이 될 것 같았습니다.

##### 수행 결과
**사용한 프롬프트**
```
부트캠프를 참여 중인, 성장하고자 하는 개발자 입장에서 흥미로워할만한 밸런스 게임 주제를 추천해줘. 
예: 배포 직전에 발견된 치명적 버그 vs 배포하고 나서 유저가 제보한 버그

머리 식힐 때 보기 좋은 가볍고 재밌는 주제였으면 좋겠어.
```

**결과**
여러 개의 밸런스 게임 주제를 추천해줬으나, 개인적으로 '부스트캠프 참여자'의 입장에선 거리가 먼 상황에 대한 밸런스 게임 주제가 다수 있었습니다. 그래서 크게 공감되거나 재밌을만한 주제가 없다고 생각해 `부트캠프 미션 수행 과정에서 공감할 수 있을만한 걸로 더 추천해줘` 라는 추가적인 요청을 보냈습니다.

##### 느낀 점
여러 번 요청을 해봤으나 100% 이게 좋겠다! 싶었던 주제는 없었습니다. 특정 집단(캠퍼)이 공감할 만한 주제를 뽑아내고 싶었는데, 아무래도 그 집단에 대한 특징, 특성에 대한 정보가 없다보니 '개발자' 라는 주제에 맞춰 포괄적인 주제를 생성해준 것 같습니다.

과제를 우선하다보니 마지막에 급하게 한 것 같아 아쉽습니다. 여러 번에 걸쳐 해보면서 프롬프트를 더 개선한다거나 사전 정보를 제공하고 요청했으면 더 공감되는 주제가 나오지 않을까 싶습니다.
